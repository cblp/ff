...
prettyTasksWikisContacts
  ...
  -> Doc AnsiStyle
prettyTasksWikisContacts isBrief tasks wiki contacts involveTasks involveWikis involveContacts =
...
prettyNote isBrief (Entity entityId Note {..}) = case isBrief of
  True -> fillSep [title text, meta]
    where
      meta = green "|" <+> cyan "id" <+> prettyDocId entityId
  False -> sparsedStack [wrapLines $ Text.pack text, sep meta]
    where
      meta =
        mconcat
...
prettyTaskSections isBrief samples =
  stack isBrief
