...
load docid = do
  Document {objectFrame = obj} <- loadDocument docid
...
loadTasks isArchived =
  filter
    ( (Just (TaskStatus $ searchStatus isArchived) ==)
        . note_status
        . entityVal
      )
    <$> loadAllNotes
...
getWikiSamplesWith predicate archive ConfigUI {shuffle} limit today =
  ...
  then pure emptySample
    ...
    do
      ...
      let wikis1 =
            filter
              (predicate . Text.pack . fromRgaM . note_text . entityVal)
              wikis0
...
updateTrackedNotes newNotes = do
  ...
  oldNotes <-
    fmap (HashMap.fromList . catMaybes)
      . for notes $ \noteId -> do
      Document {objectFrame = obj} <- loadDocument noteId
...
cmdPostpone nid = modifyAndView nid $ do
  today <- getUtcToday
  start <- fromJust <$> note_start_read
